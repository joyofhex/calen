...
class ExchangeGateway
  ...
  def room_list_addresses_for_site_code(site_code)
    room_lists.select { |k, _v| k.match(/#{site_code}/) }.values
  end
  ...
  def rooms_in_room_list(room_list_address)
    ...
    rooms.roomsArray.map do |e|
      {
        name: e[:room][:elems][:id][:elems].detect { |k, _v| k.key? :name }[:name][:text],
        address: e[:room][:elems][:id][:elems].detect { |k, _v| k.key? :email_address }[:email_address][:text]
      }
    end
  end

  def rooms_in_room_lists(room_list_addresses)
    room_list_addresses.inject([]) do |room_list, address|
  ...
  def bulk_appointments_in_range(addresses, start_time, end_time)
    ...
    addresses.each_with_index do |address, index|
      ...
      if calendar_events && calendar_events.key?(:calendar_event_array)
  ...
  def time_range_for_calendar_event(event)
    start_time = event.detect { |g| g.key? :start_time }[:start_time][:text]
    end_time = event.detect { |g| g.key? :end_time }[:end_time][:text]
